...
Until the Recepter Client library is deployed, these extra steps are necessary ot pre-build this demo

. Go to your root source folder (like `cd ~/src`)
. `git clone git@github.com:spencergibb/receptor-client.git`
. `cd receptor-client`
. `git checkout idea-java6`
. `./gradlew clean build install -x test`

With this in place, proceed to the main steps:

. Go back to the folder where you cloned lattice (like `cd ~/src`).
...
. `java -jar spring-cloud-lattice-sample/target/spring-cloud-lattice-sample-1.0.0.BUILD-SNAPSHOT.jar`
. Visit http://localhost:8080/?service=lattice-app and verify you can see the demo web app. Refresh the browser multiple times to see the *uri* attribute rotate.

NOTE: Visit link:/gs/maven/ for more background regarding Apache Maven.
